...
/* Start by constructing argument list */
	...
	else
	{
		...
		temp_string =
			(STR) ALLOCDV(type_str,
						  function->fun_temp_length + DOUBLE_ALIGN - 1);
	...
	null_flag = (USHORT)(request->req_flags & req_null);
...
/* Trap any potential errors */
	...
	if (SETJMP(env)) {
		...
		if (temp_string)
			ALL_release((FRB)temp_string);
		while (array_stack)
			ALL_free((SCHAR*)LLS_POP(&array_stack));
		ERR_punt();
	}
...
/* Process arguments */
	...
	for (tail = function->fun_rpt + 1, end = tail + function->fun_count;
		 tail < end; tail++) {
		if (tail == return_ptr)
			input = (DSC *) value;
		else
