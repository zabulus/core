 ...
 */
/*
$Id: why.c,v 1.17 2002-10-29 02:59:56 seanleyne Exp $
*/
...
#if (defined UNIX) && \
    !(defined SUPERCLIENT || defined SUPERSERVER || defined DECOSF || defined linux || defined FREEBSD || defined NETBSD || defined AIX_PPC || defined DARWIN || defined SINIXZ /* platforms without a V3 bridge */)
...
/* Special handling of dpb pointers to handle multiple extends of the dpb */
	...
	for (n = 0; n < SUBSYSTEMS; n++)
	{
											...
											expanded_filename))
		{
			...
			database = (DBB) allocate_handle(n, *handle, HANDLE_database);
			if (database)
			{
 ...
 **************************************/
	...
	if (transaction->next)
	{
		sub = allocate_handle(SUBSYSTEMS, NULL_PTR, HANDLE_transaction);
		if (!sub)
		{
			/* No memory. Make a half-hearted attempt to rollback all sub-transactions. */
