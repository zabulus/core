...
BtrPageLocks* BtrPageGCLock::getLocksCache(thread_db* tdbb)
{
	...
	if (!locks)
	{
		locks = FB_NEW (*dbb->dbb_permanent) 
			BtrPageLocks(tdbb, LCK_btr_dont_gc, sizeof(SLONG), MAX_BTR_PAGE_LOCKS);
	...
	for (; id < root->irt_count; ++id) {
			...
			(irt_desc->irt_flags & irt_in_progress) && transaction) 
		{
			...
			const int trans_state =
				TRA_wait(tdbb, transaction, trans, jrd_tra::tra_wait);
 ...
 **************************************/
	...
	Firebird::string s;
	s.printf("page %6ld splitted. split %6ld, right %6ld, parent %6ld", 
