	...
	: DataDump(pool), snapshot(pool)
{
	...
	while (reader.getRecord(dumpRecord))
	{
		...
		while (dumpRecord.getField(dumpField))
		{
			...
			// The only exception (f_mon_att_remote_process) is explicitly handled below.
			int charset = ttype_metadata;
			...
			else if (rid == rel_mon_attachments)
			{
				...
				else if (fid == f_mon_att_remote_process)
				{
					// The remote process name could be passed by the client application
					// (and thus expected to be represented in the attachment charset) or
					// generated by the client library thus having system charset.
					// As the lesser evil, let's treat it as unknown (NONE charset).
					charset = ttype_none;
				}
