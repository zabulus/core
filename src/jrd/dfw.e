...
static BOOLEAN find_depend_in_dfw(TDBB, TEXT*, USHORT, USHORT, TRA);
...
static CONST TASK task_table[] =
{
	...
	dfw_grant, (BOOLEAN(*)(TDBB, SSHORT, DFW, TRA))GRANT_privileges,
...
static BOOLEAN add_file( TDBB tdbb, SSHORT phase, DFW work, TRA transaction)
{
/**************************************
 *
 *	a d d _ f i l e
...
static BOOLEAN add_shadow( TDBB tdbb, SSHORT phase, DFW work, TRA transaction)
{
/**************************************
 *
 *	a d d _ s h a d o w
...
static BOOLEAN compute_security(TDBB tdbb,
								SSHORT phase,
								DFW work,
								TRA transaction)
{
...
static BOOLEAN create_index(TDBB tdbb,
							SSHORT phase,
							DFW work,
							TRA transaction)
{
...
static BOOLEAN create_log( TDBB tdbb, SSHORT phase, DFW work, TRA transaction)
{
/**************************************
 *
 *	c r e a t e _ l o g
...
static BOOLEAN create_procedure(TDBB	tdbb,
								SSHORT	phase,
								DFW		work,
								TRA		transaction)
{
...
static BOOLEAN create_relation(TDBB tdbb,
							   SSHORT phase,
							   DFW work,
							   TRA transaction)
{
...
static BOOLEAN create_trigger(TDBB tdbb,
							  SSHORT phase,
							  DFW work,
							  TRA transaction)
{
...
static BOOLEAN delete_exception(TDBB tdbb,
								SSHORT phase,
								DFW work,
								TRA transaction)
{
...
static BOOLEAN delete_field(TDBB tdbb,
							SSHORT phase,
							DFW work,
							TRA transaction)
{
...
static BOOLEAN delete_global(TDBB tdbb,
							 SSHORT phase,
							 DFW work,
							 TRA transaction)
{
...
static BOOLEAN delete_index(TDBB tdbb,
							SSHORT phase,
							DFW work,
							TRA transaction)
{
...
static BOOLEAN delete_log(TDBB tdbb, SSHORT phase, DFW work, TRA transaction)
{
/**************************************
 *
 *	d e l e t e _ l o g
...
static BOOLEAN delete_parameter(TDBB tdbb,
								SSHORT phase,
								DFW work,
								TRA transaction)
{
...
static BOOLEAN delete_procedure(TDBB tdbb,
								SSHORT phase,
								DFW work,
								TRA transaction)
{
...
static BOOLEAN delete_relation(TDBB tdbb,
							   SSHORT phase,
							   DFW work,
							   TRA transaction)
{
...
static BOOLEAN delete_rfr( TDBB tdbb, SSHORT phase, DFW work, TRA transaction)
{
/**************************************
 *
 *	d e l e t e _ r f r
 ...
 **************************************/
	...
	case 3:
		...
		if (relation)
		{
			id = MET_lookup_field(tdbb, relation, work->dfw_name);
			if (id >= 0)
			{
				vector = relation->rel_fields;
				if (vector &&
					(ULONG)id < vector->vec_count &&
...
static BOOLEAN delete_shadow(TDBB	tdbb,
							 SSHORT	phase,
							 DFW	work,
							 TRA	transaction)
{
...
static BOOLEAN delete_trigger(TDBB tdbb,
							  SSHORT phase,
							  DFW work,
							  TRA transaction)
{
...
static BOOLEAN find_depend_in_dfw(TDBB tdbb,
								  TEXT * object_name,
								  USHORT dep_type,
								  USHORT rel_id,
								  TRA transaction)
...
/* Compute the offsets of the various fields */
	...
	for (count = 0, desc = format->fmt_desc;
...
static BOOLEAN make_version(TDBB	tdbb,
							SSHORT	phase,
							DFW		work,
							TRA		transaction)
{
			...
			MODIFY REL USING
				...
				FOR(REQUEST_HANDLE request_fmtx)
					...
					if ((IS_DTYPE_ANY_TEXT(tfb->tfb_desc.dsc_dtype) &&
						 !INTL_defined_type(tdbb, status,
											tfb->tfb_desc.dsc_ttype)) ||
						(tfb->tfb_desc.dsc_dtype == dtype_blob &&
						 tfb->tfb_desc.dsc_sub_type == BLOB_text &&
						 !INTL_defined_type(tdbb,
											status,
											tfb->tfb_desc.dsc_scale)))
					{
						EXE_unwind(tdbb, (REQ)request_fmt1);
				...
				FOR(REQUEST_HANDLE request_fmtx)
					TRG IN RDB$TRIGGERS
						WITH TRG.RDB$RELATION_NAME = work->dfw_name AND
						(TRG.RDB$SYSTEM_FLAG = 0 OR
						 TRG.RDB$SYSTEM_FLAG MISSING)
...
static BOOLEAN modify_procedure(TDBB	tdbb,
								SSHORT	phase,
								DFW		work,
								TRA		transaction)
{
 ...
 **************************************/
	...
	case 3:
		if (!(procedure =
			 MET_lookup_procedure_id(tdbb, work->dfw_id, FALSE,
									 0))) return FALSE;
		wait = (transaction->tra_flags & TRA_nowait) ? FALSE : TRUE;
...
static BOOLEAN modify_trigger(TDBB tdbb,
							  SSHORT phase,
							  DFW work,
							  TRA transaction)
{
...
/* Go ahead and open blob */
	...
	blr = BLB_open(tdbb, dbb->dbb_sys_trans, (BID)blob_id);
	assert(blr->blb_length <= MAX_USHORT);
	length = (USHORT)blr->blb_length;
...
static BOOLEAN scan_relation(TDBB tdbb,
							 SSHORT phase,
							 DFW work,
							 TRA transaction)
{
