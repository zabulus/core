...
static BOOLEAN grantor_can_grant(GBL, TEXT *, TEXT *, TEXT *, TEXT *,
								 BOOLEAN);
 ...
 **************************************/
	...
	*p2++ = from_interp >> 8;
	...
	blob =
		(BLK) BLB_create2(tdbb, gbl->gbl_transaction, blob_id, bpb_length,
						  bpb);
	BLB_put_segment(tdbb, blob, p, length);
	BLB_close(tdbb, blob);
...
void DYN_rundown_request( JMP_BUF * old_env, BLK handle, SSHORT id)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (SETJMP(env)) {
		...
		DYN_rundown_request(old_env,
							((id == drq_s_grant || id == drq_gcg1) ? NULL :
							 request), -1);
		if (id == drq_l_grant1)
		...
		if (field[0])
		{
			...
			FOR(REQUEST_HANDLE request TRANSACTION_HANDLE gbl->gbl_transaction)
				...
				if ((PRIV.RDB$GRANT_OPTION.NULL) ||
					...
					(PRIV.RDB$GRANT_OPTION == options)) duplicate = TRUE;
				else
            ...
            *****************************************************/
			...
			if (!obj_type) {	/* relation or view because we cannot distinguish at this point. */
				...
				if (!grantor_can_grant
					(gbl, tdbb->tdbb_attachment->att_user->usr_user_name,
...
static BOOLEAN grantor_can_grant(
								 ...
								 TEXT * privilege,
								 TEXT * relation_name,
TEXT * field_name, BOOLEAN top_level)
{
/**************************************
 *
...
/* SELECT RDB$VIEW_RELATIONS/RDB$RELATION_FIELDS/... failed in grant */
	...
	FOR(REQUEST_HANDLE request TRANSACTION_HANDLE gbl->gbl_transaction)
		G_FLD IN RDB$RELATION_FIELDS CROSS
			...
			if (!DYN_REQUEST(drq_gcg3)) DYN_REQUEST(drq_gcg3) = request;
...
static void revoke( GBL gbl, UCHAR ** ptr)
{
/**************************************
 *
 *	r e v o k e
	...
	*q = 0;
	...
	if (SETJMP(env)) {
		/* we need to rundown as we have to set the env.
		   But in case the error is from store_priveledge we have already 
		   unwound the request so passing that as null */
		DYN_rundown_request(old_env, ((id == drq_s_grant) ? NULL : request),
							-1);
		if (id == drq_e_grant1)
...
static void store_privilege(
							...
							TEXT * field,
							TEXT * privilege,
SSHORT user_type, SSHORT obj_type, int option)
{
/**************************************
 *
