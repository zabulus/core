			...
			if (p < end) {
				...
				for (p++; p < end; p++) {
					if (DIGIT(*p)) {
						...
						if (exp >= SHORT_LIMIT)
							(*err) (isc_arith_except, isc_arg_gds, isc_numeric_out_of_range, isc_arg_end);
					}
	...
	if (!ts.isValid()) {
		switch (expect_type)
		{
			...
			case expect_sql_time:
				(*err)(isc_time_range_exceeded, isc_arg_end);
				break;
			case expect_timestamp:
				(*err)(isc_datetime_range_exceeded, isc_arg_end);
				break;
