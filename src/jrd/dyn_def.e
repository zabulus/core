 ...
 **************************************/
	...
	if (SETJMP(env))
	{
		...
		/* msg 121: "STORE RDB$RELATION_CONSTRAINTS failed" */
			...
			DYN_error_punt(TRUE, 124, constraint_name, NULL, NULL, NULL,
						   NULL);
			...
			if (list_ptr) {
				if (strcmp
					(((STR) list_ptr->lls_object)->str_data,
					 IDS.RDB$FIELD_NAME)) found = FALSE;
...
void DYN_define_file(
					 GBL gbl,
					 UCHAR ** ptr,
					 SLONG shadow_number, SLONG * start, USHORT msg)
{
/**************************************
 *
 ...
 **************************************/
	...
	STORE(REQUEST_HANDLE request TRANSACTION_HANDLE gbl->gbl_transaction)
		...
		if (SETJMP(env))
		{
			...
			DYN_rundown_request((JMP_BUF*)old_env, request, drq_s_funcs);
			DYN_error_punt(TRUE, 10, NULL, NULL, NULL, NULL, NULL);
			/* msg 10: "DEFINE FUNCTION failed" */
		}
	END_STORE;
	...
	STORE(REQUEST_HANDLE request TRANSACTION_HANDLE gbl->gbl_transaction)
		...
		while ((verb = *(*ptr)++) != gds__dyn_end)
			...
			case gds__dyn_fld_edit_string2:
				DYN_get_string2(ptr, FLD.RDB$EDIT_STRING,
 ...
 **************************************/
	...
	UCHAR index_name[32], referenced_relation[32];
	...
	STORE(REQUEST_HANDLE request TRANSACTION_HANDLE gbl->gbl_transaction)
		...
		if (!GET_STRING(ptr, IDX.RDB$INDEX_NAME))
			DYN_UTIL_generate_index_name(tdbb, gbl, IDX.RDB$INDEX_NAME,
										 index_type);
		...
		while ((verb = *(*ptr)++) != gds__dyn_end)
			...
			case gds__dyn_fld_name:
				str = (STR) ALLOCDV(type_str, field_name_size - 1);
				DYN_get_string(ptr, str->str_data, field_name_size, TRUE);
			...
			FOR(REQUEST_HANDLE request TRANSACTION_HANDLE gbl->gbl_transaction)
				...
				if (list_ptr) {
					...
					DYN_terminate(((STR) list_ptr->lls_object)->str_data,
								  ...
								  str_length);
					if (strcmp(((STR) list_ptr->lls_object)->str_data,
								Z.RDB$FIELD_NAME))
					{
						found = FALSE;
					}
...
#if (defined JPN_SJIS || defined JPN_EUC)
			case gds__dyn_fld_edit_string2:
				DYN_get_string2(ptr, RFR.RDB$EDIT_STRING,
...
void DYN_define_trigger(
						...
						UCHAR ** ptr,
						TEXT * relation_name,
						TEXT * trigger_name, BOOLEAN ignore_perm)
{
/**************************************
 *
...
static void check_unique_name(
							  TDBB tdbb,
							  GBL gbl, TEXT * object_name, BOOLEAN proc_flag)
{
/**************************************
 *
...
static BOOLEAN find_field_source(
								 ...
								 TEXT * view_name,
								 USHORT context,
								 TEXT * local_name, TEXT * field_name)
{
/**************************************
 *
