...
static void save_security_class(TDBB, TEXT *, UCHAR *, USHORT);
...
/* realloc the new length, ERR_punt incase of no memory */
	(void) ALL_extend((BLK*)&start_ptr, realloc_length);
	...
	case 3:
		{
			...
			if (SETJMP(env)) {
				...
				if (str_default_buffer)
					ALL_release((FRB)str_default_buffer);
				ERR_punt();
			}
			...
			if (work->dfw_id == obj_relation) {
				...
				if (length > default_length) {
					(void) ALL_extend((BLK*)&str_default_buffer, length);
 ...
 **************************************/
	...
	ULONG *field_length_ptr = &field_length, *start_length_ptr =
		&start_length;
	...
	if (SETJMP(env)) {
		tdbb->tdbb_setjmp = (UCHAR *) old_env;
		if (str_field_buffer)
			ALL_release((FRB)str_field_buffer);
		if (str_field_buffer_start)
			ALL_release((FRB)str_field_buffer_start);
		ERR_punt();
	}
/* initialize the field-level acl buffer to include all relation-level privs */

	str_field_buffer_start = (STR)ALLOCPV(type_str, *length_ptr);
	str_field_buffer       = (STR)ALLOCPV(type_str, *length_ptr);
...
static USHORT terminate( TEXT * string)
{
/**************************************
 *
 *	t e r m i n a t e
...
static BOOLEAN check_string( TEXT * acl, TEXT * string)
{
/**************************************
 *
