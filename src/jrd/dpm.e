 ...
 **************************************/
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("    record  %d:%d transaction %d back %d:%d fragment %d:%d flags %d\n",
	...
	*index1 = *index2;
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("    old record %d:%d, new record %d:%d, old dpg_count %d, ",
 ...
 **************************************/
	...
	if (debug_flag > DEBUG_WRITES_INFO) {
		ib_printf
			("    org record  %d:%d transaction %d back %d:%d fragment %d:%d flags %d\n",
...
void DPM_create_relation( TDBB tdbb, REL relation)
{
/**************************************
 *
...
SLONG DPM_data_pages(TDBB tdbb, REL relation)
{
/**************************************
 *
...
/* Compute the highest line number level on page */
	...
	for (index = &page->dpg_rpt[page->dpg_count];
		 index > page->dpg_rpt; --index) if (index[-1].dpg_offset)
			break;
...
/* If the page is not empty and used to be marked as full, change the
   state of both the page and the appropriate pointer page. */
	...
	if (count && (page->dpg_header.pag_flags & dpg_full)) {
		DEBUG page->dpg_header.pag_flags &= ~dpg_full;
		mark_full(tdbb, rpb);
...
/* Data page is still empty and still in the relation.  Eliminate the
   pointer to the data page then release the page. */
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("\tDPM_delete:  page %d is empty and about to be released from relation %d\n",
...
/* Make sure that the pointer page is written after the data page.
   The resulting 'must-be-written-after' graph is:
	...
	for (ptr = &ppage->ppg_page[ppage->ppg_count]; ptr > ppage->ppg_page;
		 --ptr) if (ptr[-1])
			break;
...
void DPM_delete_relation( TDBB tdbb, REL relation)
{
/**************************************
 *
...
/* Delete all data and pointer pages */
	...
	{
		ppage = get_pointer_page(tdbb, relation, &window, sequence, LCK_read);
		if (!ppage)
		{
		...
		for (i = 0; i < ppage->ppg_count; i++, page++)
		{
			...
			if (flags[i >> 2] & (2 << ((i & 3) << 1)))
			{
				...
				dpage =
					(DPG) CCH_FETCH(tdbb, &data_window, LCK_write, pag_data);
				line = dpage->dpg_rpt;
...
BOOLEAN DPM_fetch(TDBB tdbb, register RPB * rpb, USHORT lock)
{
/**************************************
 *
 *	D P M _ f e t c h
...
SSHORT DPM_fetch_back(TDBB tdbb,
					  register RPB * rpb, USHORT lock, SSHORT latch_wait)
{
/**************************************
 *
...
void DPM_fetch_fragment( TDBB tdbb, register RPB * rpb, USHORT lock)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (debug_flag > DEBUG_READS_INFO)
		ib_printf
			("    record fetched  %d:%d transaction %d back %d:%d fragment %d:%d flags %d\n",
			 rpb->rpb_page, rpb->rpb_line, rpb->rpb_transaction,
			 rpb->rpb_b_page, rpb->rpb_b_line, rpb->rpb_f_page,
			 rpb->rpb_f_line, rpb->rpb_flags);
#endif
 ...
 **************************************/
	...
	if (!(vector = dbb->dbb_gen_id_pages) ||
		(sequence >= vector->vcl_count) || !(vector->vcl_long[sequence])) {
		DPM_scan_pages(tdbb);
		if (!(vector = dbb->dbb_gen_id_pages) ||
			(sequence >= vector->vcl_count) || !(vector->vcl_long[sequence])) {
			...
			if (!(vector = dbb->dbb_gen_id_pages)) {
				vector = dbb->dbb_gen_id_pages =
					(VCL) ALLOCPV(type_vcl, sequence + 1);
				vector->vcl_count = sequence + 1;
			}
			else if (sequence >= vector->vcl_count)
			{
				vector = (VCL)ALL_extend((BLK*)&dbb->dbb_gen_id_pages, sequence + 1);
			}
...
int DPM_get( TDBB tdbb, register RPB * rpb, SSHORT lock_type)
{
/**************************************
 *
 *	D P M _ g e t
 ...
 **************************************/
	...
	if (debug_flag > DEBUG_READS)
		ib_printf
			("DPM_get_blob (blob, record_number %d, delete_flag %d, prior_page %d)\n",
	...
	if (blob->blb_level == 0) {
		...
		if (!(vector = blob->blb_pages)) {
			...
			vector = blob->blb_pages =
				(VCL) ALLOCTV(type_vcl, blob->blb_max_pages);
		}
				...
				if (!onepage && !line && !backwards) {
					...
					if (!(slot % dbb->dbb_prefetch_sequence)) {
						...
						for (i = 0;
							 ...
							 && slot2 < ppage->ppg_count;) pages[i++] =
								ppage->ppg_page[slot2++];
...
#endif
				dpage =
					(DPG) CCH_HANDOFF(tdbb, window, page_number, lock_type,
									  pag_data);
...
#ifdef SUPERSERVER_V2
SLONG DPM_prefetch_bitmap(TDBB tdbb, REL relation, SBM bitmap, SLONG number)
{
/**************************************
 *
...
/* Special case update of RDB$PAGES pointer page vector to avoid
   ...
   has been extended with another pointer page. */
	...
	while (ppage->ppg_next) {
		++sequence;
		vector = (VCL) ALL_extend((BLK*)address, sequence + 1);
		vector->vcl_long[sequence] = ppage->ppg_next;
		ppage =
			(PPG) CCH_HANDOFF(tdbb, &window, ppage->ppg_next, LCK_read,
							  pag_pointer);
	}
	...
	FOR(REQUEST_HANDLE request) X IN RDB$PAGES
		...
		if (!(vector = *address)) {
			...
			vector = (VCL) ALL_extend((BLK*)address, n);
...
void DPM_store( TDBB tdbb, register RPB * rpb, LLS * stack, USHORT type)
{
/**************************************
 *
 *	D P M _ s t o r e
 ...
 **************************************/
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("    record to store %d:%d transaction %d back %d:%d fragment %d:%d flags %d\n",
			 rpb->rpb_page, rpb->rpb_line, rpb->rpb_transaction,
			 rpb->rpb_b_page, rpb->rpb_b_line, rpb->rpb_f_page,
			 rpb->rpb_f_line, rpb->rpb_flags);
#endif
...
/* If the record isn't going to fit on a page, even if fragmented,
   handle it a little differently. */
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("    record %d:%d, length %d, rpb_flags %d, f_page %d:%d, b_page %d:%d\n",
...
/* Figure out precedence pages, if any */
	...
	if (blob->blb_level > 0)
		for (ptr = vector->vcl_long, end = ptr + vector->vcl_count;
			 ptr < end;) LLS_PUSH((BLK) * ptr++, &stack);

/* Locate space to store blob */

	rpb.rpb_relation = blob->blb_relation;
	header = (BLH) locate_space(tdbb,
								&rpb,
 ...
 **************************************/
	...
	if (debug_flag > DEBUG_WRITES_INFO) {
		ib_printf
			("    old flags %d, old transaction %d, old format %d, old back record %d:%d\n",
			 header->rhd_flags, header->rhd_transaction, header->rhd_format,
			 header->rhd_b_page, header->rhd_b_line);
		ib_printf
			("    new flags %d, new transaction %d, new format %d, new back record %d:%d\n",
...
void DPM_update( TDBB tdbb, register RPB * rpb, LLS * stack, TRA transaction)
{
/**************************************
 *
 *	D P M _ u p d a t e
...
/* Find space on page */
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("    record %d:%d, dpg_length %d, rpb_flags %d, rpb_f record %d:%d, rpb_b record %d:%d\n",
...
static void fragment(
					 ...
					 DCC dcc, SSHORT length, TRA transaction)
{
/**************************************
 *
 *	f r a g m e n t
 ...
 **************************************/
	...
	if (debug_flag > DEBUG_WRITES)
		ib_printf
			("fragment (rpb %d, available_space %d, dcc, length %d, transaction %d)\n",
...
/* Start by claiming what space exists on the page.  Note that
   ...
   new version. */
	...
	if (debug_flag > DEBUG_WRITES_INFO)
		ib_printf
			("    rhdf_transaction %d, window record %d:%d, available_space %d, rhdf_flags %d, rhdf_f record %d:%d, rhdf_b record %d:%d\n",
...
static void extend_relation( TDBB tdbb, REL relation, WIN * window)
{
/**************************************
 *
  ...
  retry_after_latch_timeout:
	...
	for (pp_sequence = relation->rel_slot_space;; pp_sequence++) {
		...
		if (ppage->ppg_header.pag_flags & ppg_eof) {
			...
			vector = (VCL) ALL_extend((BLK*)&relation->rel_pages, pp_sequence + 1);
			...
			ppage =
				(PPG) CCH_HANDOFF(tdbb, &pp_window, new_pp_window.win_page,
...
/* In the case of a timeout, retry the whole thing. */
	...
	if (dbb->dbb_wal) {
		...
		CCH_journal_record(tdbb, &pp_window, (UCHAR*)&journal, JRNP_SIZE,
						   (UCHAR*)&window->win_page, sizeof(window->win_page));
	}
...
static BOOLEAN get_header( WIN * window, SSHORT line, register RPB * rpb)
{
/**************************************
 *
 *	g e t _ h e a d e r
 ...
 **************************************/
	...
	if (!(vector = relation->rel_pages) || sequence >= vector->vcl_count) {
		for (;;) {
			...
			if (!next_ppg)
				return NULL;
			DPM_pages(tdbb, relation->rel_id, pag_pointer, vector->vcl_count,
					  next_ppg);
		}
	}
...
static void journal_segment( TDBB tdbb, WIN * window, USHORT slot)
{
/**************************************
 *
 ...
 **************************************/
	...
	CCH_journal_record(tdbb, window, (UCHAR*)&journal, JRNP_SIZE,
...
/* If there is a preferred page, try there first */
	...
	if (type == DPM_secondary) {
		...
		DECOMPOSE(sequence, dbb->dbb_dp_per_pp, pp_sequence, slot);
		if (ppage =
			get_pointer_page(tdbb, relation, window, pp_sequence,
							 LCK_read)) if (slot < ppage->ppg_count
											&& (dp_number =
												ppage->ppg_page[slot])) {
...
static void mark_full( TDBB tdbb, register RPB * rpb)
{
/**************************************
 *
 *	m a r k _ f u l l
...
/* Fetch the pointer page, then the data page.  Since this is a case of
   ...
   the whole thing. */
	...
	do {
		...
		dpage =
			(DPG) CCH_FETCH_TIMEOUT(tdbb, &rpb->rpb_window, LCK_read,
									pag_data, -1);
...
static void release_dcc( DCC dcc)
{
/**************************************
 *
...
/* What's left fits on a page.  Luckily, we don't have to store it
   ourselves. */
	...
	if (debug_flag > DEBUG_WRITES_INFO) {
		...
		ib_printf
			("    rhdf_trans %d, rpb_window record %d:%d, dpg_length %d \n\trhdf_flags %d, rhdf_f record %d:%d, rhdf_b record %d:%d\n",
