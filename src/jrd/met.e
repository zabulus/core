...
static void get_trigger(TDBB, REL, SLONG[2], VEC *, TEXT *, BOOLEAN, USHORT);
...
void MET_change_fields( TDBB tdbb, TRA transaction, DSC * field_source)
{
/**************************************
 *
...
FMT MET_current(TDBB tdbb, REL relation)
{
/**************************************
 *
...
FMT MET_format(TDBB tdbb, register REL relation, USHORT number)
{
/**************************************
 *
 *      M E T _ f o r m a t
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE request)
		...
		BLB_get_data(tdbb, blob, (UCHAR*)format->fmt_desc, blob->blb_length);

		for (desc = format->fmt_desc + count - 1;
...
NOD MET_get_dependencies(TDBB			tdbb,
						 REL			relation,
						 TEXT*			blob,
						 CSB			view_csb,
						 SLONG			blob_id[2],
...
FLD MET_get_field(REL relation, USHORT id)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (!relation ||
		...
		id >= vector->vec_count) return NULL;
...
int MET_get_walinfo(
					TDBB tdbb,
					LGFILE ** logfiles, ULONG * number, LGFILE ** over_flow)
{
/**************************************
 *
...
void MET_load_trigger(
					  TDBB tdbb,
					  REL relation, TEXT * trigger_name, VEC * triggers)
{
/**************************************
 *
...
/* Start by checking field names that we already know */
	...
	{
		length = strlen(name);
		for (field = (FLD*) vector->vec_object,
				id = 0,
				end = field + vector->vec_count;
			field < end;
			field++, id++)
		{
			if (*field &&
...
int MET_lookup_partner(
					   TDBB tdbb, REL relation, IDX * idx, UCHAR * index_name)
{
/**************************************
 *
	...
	if (relation->rel_flags & REL_check_partners)
	{
		...
		FOR(REQUEST_HANDLE request)
			...
			if (partner_relation)
			{
				...
				references->frgn_relations->vec_object[index_number] =
				...
				references->frgn_indexes->vec_object[index_number] =
...
*/
		...
		FOR(REQUEST_HANDLE request)
				...
				MET_lookup_relation(tdbb, IND.RDB$RELATION_NAME))
			{
				...
				dependencies->prim_reference_ids->vec_object[index_number] =
					(BLK) (IDX.RDB$INDEX_ID - 1);
				...
				dependencies->prim_relations->vec_object[index_number] =
					(BLK) partner_relation->rel_id;
				...
				dependencies->prim_indexes->vec_object[index_number] =
					(BLK) (IND.RDB$INDEX_ID - 1);
	...
	if (idx->idx_flags & idx_foreign)
	{
		...
		{
			for (index_number = 0;
				 index_number < references->frgn_reference_ids->vec_count;
				 index_number++)
			{
				if (idx->idx_id == (UCHAR) references->frgn_reference_ids->
											vec_object[index_number])
				{
					idx->idx_primary_relation =
						(USHORT) references->frgn_relations->
						vec_object[index_number];
					idx->idx_primary_index =
						(UCHAR) references->frgn_indexes->
						vec_object[index_number];
					return TRUE;
				}
			}
	...
	else if (idx->idx_flags & (idx_primary | idx_unique))
	{
		...
		{
			for (index_number = 0;
				 index_number < dependencies->prim_reference_ids->vec_count;
				 index_number++)
			{
...
PRC MET_lookup_procedure(TDBB tdbb, SCHAR * name)
{
/**************************************
 *
...
PRC MET_lookup_procedure_id(TDBB tdbb,
							SSHORT id, BOOLEAN return_deleted, USHORT flags)
{
/**************************************
 *
...
/* See if we already know the relation by name */
	...
	if (procedures = dbb->dbb_procedures)
		for (ptr = (PRC *) procedures->vec_object, end =
			 ptr + procedures->vec_count; ptr < end; ptr++) {
			if ((procedure = *ptr) && procedure->prc_id == id
				&& !(procedure->prc_flags & PRC_being_scanned)
				&& (procedure->prc_flags & PRC_scanned)
				&& !(procedure->prc_flags & PRC_being_altered)
...
/* See if we already know the relation by name */
	...
	for (ptr = (REL *) relations->vec_object, end =
		 ptr + relations->vec_count; ptr < end; ptr++)
	{
		if ((relation = *ptr) &&
...
REL MET_lookup_relation_id(TDBB tdbb, SLONG id, BOOLEAN return_deleted)
{
/**************************************
 *
...
/* System relations are above suspicion */
	...
	if ((vector = dbb->dbb_relations) &&
		(id < vector->vec_count) && (relation = (REL) vector->vec_object[id]))
	{
...
void MET_parse_sys_trigger(TDBB tdbb, REL relation)
{
/**************************************
 *
...
/* No need to load triggers for ReadOnly databases, since
   INSERT/DELETE/UPDATE statements are not going to be allowed */
	...
	trigger = NULL;
	/* TMN: The cast "(struct trg*)" if a bad lie. Fix later
...
int MET_post_existence( TDBB tdbb, REL relation)
{
/**************************************
 *
...
void MET_prepare( TDBB tdbb, TRA transaction, USHORT length, UCHAR * msg)
{
/**************************************
 *
...
PRC MET_procedure(TDBB tdbb, int id, USHORT flags)
{
/**************************************
 *
 ...
 **************************************/
	...
	{
		vector = (VEC) ALL_extend((BLK*)&dbb->dbb_procedures, id + 10);
	}
	...
	procedure = (PRC) vector->vec_object[id];
	if (procedure)
	{
	...
	FOR(REQUEST_HANDLE request)
		...
		FOR(REQUEST_HANDLE request2) PA IN RDB$PROCEDURE_PARAMETERS CROSS
			...
			parameter =
				(PRM) ALLOCPV(type_prm, name_length(PA.RDB$PARAMETER_NAME));
...
REL MET_relation(TDBB tdbb, USHORT id)
{
/**************************************
 *
 ...
 **************************************/
	...
	{
		vector = (VEC) ALL_extend((BLK*)&dbb->dbb_relations, id + 10);
	}
...
void MET_remove_procedure( TDBB tdbb, int id, PRC procedure)
{
/**************************************
 *
 ...
 */
	...
	if (procedure->prc_existence_lock)
		ALL_release((FRB)procedure->prc_existence_lock);

	if (procedure->prc_name)
		ALL_release((FRB)procedure->prc_name);
	if (procedure->prc_security_name)
		ALL_release((FRB)procedure->prc_security_name);
...
/* deallocate input param structures */
	...
	if ((procedure->prc_inputs) && (vector = procedure->prc_input_fields)) {
		for (i = 0; i < procedure->prc_inputs; i++)
		{
			...
			{
				ALL_release((FRB)vector->vec_object[i]);
			}
		}
		ALL_release((FRB)vector);
	}
...
/* deallocate output param structures */
	...
	if (procedure->prc_format)
		ALL_release((FRB)procedure->prc_format);

	if (!(procedure->prc_flags & PRC_being_altered))
	{
		ALL_release((FRB)procedure);
	}
	else
	{
...
void MET_scan_relation( TDBB tdbb, REL relation)
{
/**************************************
 *
...
/* Since this can be called recursively, find an inactive clone of the request */
	...
	FOR(REQUEST_HANDLE request)
		...
		vector =
			ALL_vector(dbb->dbb_permanent, &relation->rel_fields,
					   REL.RDB$FIELD_ID);
...
TEXT *MET_trigger_msg(TDBB tdbb, TEXT * name, USHORT number)
{
/**************************************
 *
...
void MET_update_transaction( TDBB tdbb, TRA transaction, USHORT flag)
{
/**************************************
 *
...
static void blocking_ast_procedure( PRC procedure)
{
/**************************************
 *
...
static void blocking_ast_relation( REL relation)
{
/**************************************
 *
...
static void lookup_view_contexts( TDBB tdbb, REL view)
{
/**************************************
 *
...
static NOD parse_procedure_blr(
							   TDBB tdbb,
							   PRC procedure, SLONG blob_id[2], CSB * csb_ptr)
{
/**************************************
 *
...
void MET_release_triggers( TDBB tdbb, VEC * vector_ptr)
{
/***********************************************
 *
...
static void save_trigger_request(DBB dbb, VEC * ptr, REQ request)
{
/**************************************
 *
		...
		if (field_node)
		{
			if (field_node->nod_type == nod_field)
			{
				...
				if (relation)
				{
					...
					{
						field =
							(FLD) procedure->prc_output_fields->
							vec_object[fld_id];
					}
				}
			}
