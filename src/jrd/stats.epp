 ...
 **************************************/
	...
	if (gds__database_info(GDS_VAL(status_vector),
						   GDS_VAL(db_handle),
						   sizeof(info_request),
						   info_request,
						   sizeof(info_buffer),
						   info_buffer)) return status_vector[1];
	...
	for (p = info_buffer;
		 p < info_buffer + sizeof(info_buffer) && *p != gds__info_end;) {
 ...
 **************************************/
	...
	if (!*arg) {
		*arg = 1;
		...
		if (n = count)
			do
				ib_printf("%10s", *headers++);
			while (--n);
		ib_printf("\n");
	}

	for (request_handle request_handle)
		x in rdb$relations with x.rdb$relation_id eq relation_id
