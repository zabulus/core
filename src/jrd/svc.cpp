	...
	svc_stdin_preload_requested(0), svc_stdin_user_size(0)
{
	...
	catch (const Firebird::Exception& ex)
	{
		...
		try
		{
			...
			if (trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_ATTACH))
			{
				...
				trace_manager->event_service_attach(&service, no_priv ? Firebird::ITraceConnection::TRACE_RESULT_UNAUTHORIZED : Firebird::ITraceConnection::TRACE_RESULT_FAILED);
			}
	...
	if (svc_trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_ATTACH))
	{
		TraceServiceImpl service(this);
		svc_trace_manager->event_service_attach(&service, Firebird::ITraceConnection::TRACE_RESULT_SUCCESS);
	}
}
...
void Service::detach()
{
	...
	TraceServiceImpl service(this);
	svc_trace_manager->event_service_detach(&service, Firebird::ITraceConnection::TRACE_RESULT_SUCCESS);
						   ...
						   UCHAR* info)
{
	...
	if (svc_trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_QUERY))
	{
		TraceServiceImpl service(this);
		svc_trace_manager->event_service_query(&service, send_item_length, send_items,
			recv_item_length, recv_items, Firebird::ITraceConnection::TRACE_RESULT_SUCCESS);
	}
	...
	catch (const Firebird::Exception& ex)
	{
		...
		if (svc_trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_QUERY))
		{
			...
			TraceServiceImpl service(this);
			svc_trace_manager->event_service_query(&service, send_item_length, send_items,
				recv_item_length, recv_items, (no_priv ? Firebird::ITraceConnection::TRACE_RESULT_UNAUTHORIZED : Firebird::ITraceConnection::TRACE_RESULT_FAILED));
		}
		throw;
	}
					...
					UCHAR*			info)
{
	...
	{
		if ((svc_flags & SVC_detached) &&
			svc_trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_QUERY))
		{
			TraceServiceImpl service(this);
			svc_trace_manager->event_service_query(&service, send_item_length, send_items,
				recv_item_length, recv_items, Firebird::ITraceConnection::TRACE_RESULT_SUCCESS);
...
void Service::start(USHORT spb_length, const UCHAR* spb_data)
{
	...
	***/
	...
	{
		if (svc_trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_START))
		{
			ISC_STATUS_ARRAY status_vector;
			const ISC_STATUS exc = ex.stuff_exception(status_vector);
			const bool no_priv = (exc == isc_login || exc == isc_no_priv);
			...
			svc_trace_manager->event_service_start(&service,
				...
				no_priv ? Firebird::ITraceConnection::TRACE_RESULT_UNAUTHORIZED : Firebird::ITraceConnection::TRACE_RESULT_FAILED);
		}
		throw;
	}

	if (this->svc_trace_manager->needs(Firebird::ITraceConnection::TRACE_EVENT_SERVICE_START))
	{
		TraceServiceImpl service(this);
		this->svc_trace_manager->event_service_start(&service,
			this->svc_switches.length(), this->svc_switches.c_str(),
			this->svc_status[1] ? Firebird::ITraceConnection::TRACE_RESULT_FAILED : Firebird::ITraceConnection::TRACE_RESULT_SUCCESS);
	}
}
