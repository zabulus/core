...
/* Perform some special handling when run as an Interbase service.  The
   ...
   by 3 file descriptors to use in re-directing ib_stdin, ib_stdout, and ib_stderr. */
	...
	name = NULL;
	sw_log = sw_system = sw_data = sw_index = sw_version = sw_header =
		sw_record = sw_relation = FALSE;
...
/* print continuation file sequence */
	...
	dba_print(9, current->fil_string,
			  (current == tddba->files) ? "only" : "last", 0, 0, 0);	/* msg 9: File %s is the %s file\n */
 ...
 **************************************/
	...
	if (tddba->files) {
		...
		fil->fil_next =
			(DBA_FIL) ALLOC(sizeof(struct dba_fil) +
...
#endif /* READONLY_DATABASE */
	{
		...
		CMD_UTIL_put_svc_status(tddba->dba_service_blk->svc_status,
								...
								0, NULL, 0, NULL, 0, NULL, 0, NULL);	/* msg 29: Can't open database file %s */
#endif
		db_error(errno);
	}
