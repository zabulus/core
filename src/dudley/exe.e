...
/* Generate a dpb for attempting to attach to, and
	...
	*d++ = gds__dpb_version1;
	...
	if (!DDL_replace) {
		if (!result) {
			gds__detach_database(gds__status, GDS_REF(DB));
 ...
 **************************************/
	...
	*d++ = gds__dpb_version1;
	...
	COMMIT ON_ERROR DDL_db_error(gds__status, 26, NULL, NULL, NULL, NULL, NULL);	/* msg 26: error commiting metadata changes */
	ROLLBACK;
	END_ERROR;
	...
	for (file = dbb->dbb_files; file; file = file->fil_next)
		if (unlink(file->fil_name->sym_name))
			DDL_error_abort(NULL_PTR, 28, file->fil_name->sym_name, NULL,
							NULL, NULL, NULL);	/* msg 28: Could not delete file %s */

	if (unlink(dbb->dbb_name->sym_string))
		DDL_error_abort(NULL_PTR, 28, dbb->dbb_name->sym_string, NULL, NULL,
						NULL, NULL);	/* msg 28: Could not delete file %s */
}
 ...
 **************************************/
	...
	if (DB) {
		if (DDL_errors) {
			...
			ROLLBACK
				ON_ERROR DDL_db_error(gds__status, 35, NULL, NULL, NULL, NULL, NULL);	/* msg 35: error rolling back metadata changes */
		END_ERROR}
		else {
			COMMIT
				ON_ERROR DDL_db_error(gds__status, 36, NULL, NULL, NULL, NULL, NULL);	/* msg 36: error commiting metadata changes */
			ROLLBACK;
		END_ERROR}
		FINISH;
	}
 ...
 **************************************/
	...
	*d++ = gds__dpb_version1;
	...
	if (gds__attach_database(gds__status, 0,
							 ...
							 dpb_length,
							 dpb))
			DDL_error_abort(gds__status, 29, dbb->dbb_name->sym_string, NULL,
...
/* erase log files and commit transaction */
	...
	if (dbb->dbb_flags & DBB_drop_log) {
		...
		END_FOR;
		COMMIT ON_ERROR DDL_db_error(gds__status, 321, NULL, NULL, NULL, NULL, NULL);	/* msg 321: error commiting new write ahead log declarations */
		ROLLBACK;
		END_ERROR;
		START_TRANSACTION;
...
/* Unless there are errors, commit the new shared cache immediately! */
	...
	if (!DDL_errors) {
		COMMIT ON_ERROR DDL_db_error(gds__status, 324, NULL, NULL, NULL, NULL, NULL);	/* msg 324: error commiting new shared cache declaration */
		ROLLBACK;
		END_ERROR;
		START_TRANSACTION;
	}
...
static void add_field( FLD field)
{
/**************************************
 *
 *	a d d _ f i e l d
   ...
   against the user given maximum length (if given). */
	...
	if (dbb) {
		if (gds__database_info(gds__status,
							   ...
							   sizeof(s),
							   s) ||
			s[0] != gds__info_allocation) DDL_err(38, NULL, NULL, NULL, NULL, NULL);	/* msg 38: gds__database_info failed */
...
/* Unless there are errors floating around, commit the new file immediately! */
	...
	if (!DDL_errors) {
		COMMIT ON_ERROR DDL_db_error(gds__status, 40, NULL, NULL, NULL, NULL, NULL);	/* msg 40: error commiting new file declarations */
		ROLLBACK;
		END_ERROR;
		START_TRANSACTION;
	}
...
static void add_global_field( FLD field)
{
/**************************************
 *
 ...
 **************************************/
	...
	size = 0;
	for (i = 0, symbol = index->idx_field; i < index->idx_count;
		 i++, symbol++) {
		if (!(field = check_field(index->idx_relation, *symbol))) {
			DDL_err(44, index->idx_name->sym_string,
					(*symbol)->sym_string, index->idx_relation->sym_string,
					NULL, NULL);
	...
	if ((index->idx_count == 1) && size > 254) {
		DDL_err(46, (TEXT *) size, index->idx_name->sym_string, NULL, NULL,
				NULL);
		/* msg 46: combined key length (%d) for index %s is > 254 bytes */
		return;
	}
	else if ((index->idx_count > 1) && size > 202) {
		DDL_err(312, (TEXT *) size, index->idx_name->sym_string, NULL, NULL,
				NULL);
	...
	if (index->idx_description) {
		...
		X.RDB$DESCRIPTION.NULL = TRUE;
	END_STORE ON_ERROR if (gds__status[1] == gds__no_dup) {
		DDL_err(47, index->idx_name->sym_string, NULL, NULL, NULL, NULL);	/* msg 47: index %s already exists */
		return;
	}

	{
		DDL_db_error(gds__status, 48, index->idx_name->sym_string, NULL, NULL,
					 NULL, NULL);
...
static void add_relation( REL relation)
{
/**************************************
 *
...
static void add_security_class( SCL class)
{
/**************************************
 *
...
static void add_trigger( TRG trigger)
{
/**************************************
 *
...
static void add_view( REL relation)
{
/**************************************
 *
 *	a d d _ v i e w
...
static FLD check_field( SYM relation, SYM field)
{
/**************************************
 *
...
static BOOLEAN check_range( FLD field)
{
/**************************************
 *
...
static void close_blob( void *blob)
{
/**************************************
 *
 *	c l o s e _ b l o b
...
static void *create_blob( SLONG * blob_id, USHORT bpb_length, UCHAR * bpb)
{
/**************************************
 *
...
static void drop_field( FLD field)
{
/**************************************
 *
 *	d r o p _ f i e l d
...
static void drop_global_field( FLD field)
{
/**************************************
 *
...
static void drop_relation( REL relation)
{
/**************************************
 *
...
static void drop_security_class( SCL class)
{
/**************************************
 *
...
static void drop_trigger( TRG trigger)
{
/**************************************
 *
...
static void get_field_desc( FLD field)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR X IN RDB$FIELDS 
	    ...
	    field->fld_name = symbol =
            get_symbol(SYM_global, X.RDB$FIELD_NAME, field);
        HSH_insert(symbol);
...
static void get_log_names(SCHAR * db_name,
						  FIL * start_file,
						  SCHAR * cur_log,
...
static void get_log_names_serial( FIL * start_file)
{
/**************************************
 *
...
static void get_relations( DBB database)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR R IN RDB$RELATIONS 
	    ...
	    relation->rel_name = symbol =
            get_symbol(SYM_relation, R.RDB$RELATION_NAME, relation);
        HSH_insert(symbol);
 ...
 **************************************/
	...
	isc_start_and_send((SLONG *) 0, &req_handle, &gds__trans, 0, sizeof(msg),
					   &msg, 0);
...
static SYM get_symbol( enum sym_t type, TEXT * string, CTX object)
{
/**************************************
 *
 *	g e t _ s y m b o l
...
static void get_triggers( DBB database)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR T IN RDB$TRIGGERS WITH T.RDB$SYSTEM_FLAG MISSING OR 
	    ...
	    trigger->trg_name = symbol =
            get_symbol(SYM_trigger, T.RDB$TRIGGER_NAME, trigger);
        HSH_insert(symbol);
        rel_name =
        ...
        if (rel_name && rel_name->sym_object)
            ...
            trigger->trg_relation =
                (REL) get_symbol(SYM_relation, T.RDB$RELATION_NAME, NULL);
 ...
 **************************************/
	...
	FOR FUN IN RDB$FUNCTIONS CROSS ARG IN RDB$FUNCTION_ARGUMENTS WITH
        ...
        function->func_name = symbol =
            get_symbol(SYM_function, FUN.RDB$FUNCTION_NAME, function);
        HSH_insert(symbol);
...
static void make_desc( NOD node, DSC * desc)
{
/**************************************
 *
 *	m a k e _ d e s c
...
static void modify_field( FLD field)
{
/**************************************
 *
...
static void modify_global_field( FLD field)
{
/**************************************
 *
...
static void modify_relation( REL relation)
{
/**************************************
 *
...
static void modify_trigger( TRG trigger)
{
/**************************************
 *
...
static void move_symbol( SYM symbol, register SCHAR * field, SSHORT length)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (symbol) {
		if ((l = symbol->sym_length) > length) {
			DDL_err(296, symbol->sym_string, (TEXT *) length, NULL, NULL,
					NULL);
...
static void set_generator( NOD node)
{
/**************************************
 *
...
/* For V4.0 we don't care about Gdef specific memory leaks */
	...
	isc_receive((SLONG *) 0, &req_handle1, 0, sizeof(gen_value), &gen_value,
				0);
...
/* For V4.0 we don't care about Gdef specific memory leaks */
	...
	for (i = 5; (gen_value != new) && --i;) {
		*number = new - gen_value;
		...
		isc_compile_request2((SLONG *) 0, &DB, &req_handle2, l, blr);
		isc_start_request((SLONG *) 0, &req_handle2, &gds__trans, 0);
		isc_receive((SLONG *) 0, &req_handle2, 0, sizeof(gen_value),
					&gen_value, 0);
		isc_release_request((SLONG *) 0, &req_handle2);
	}
...
static void store_acl( SCL class, SLONG * blob_id)
{
/**************************************
 *
 *	s t o r e _ a c l
...
static void store_blr( NOD node, SLONG * blob_id, REL relation)
{
/**************************************
 *
 *	s t o r e _ b l r
...
static void store_query_header( NOD node, SLONG * blob_id)
{
/**************************************
 *
...
static void store_range( FLD field)
{
/**************************************
 *
...
static void store_text( TXT text, SLONG * blob_id)
{
/**************************************
 *
 *	s t o r e _ t e x t
