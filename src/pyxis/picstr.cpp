...
extern double PASCAL_ROUTINE	MOVQ_get_double(DSC*);
#endif
...
if (c == '?')
    picture->pic_missing = PIC_analyze (picture->pic_pointer, (DSC*) NULL_PTR);
...
switch (picture->pic_type)
    {
    case pic_alpha:	edit_alpha (desc, picture, output, max_length); return 0;
    case pic_numeric:	edit_numeric (desc, picture, output); return 0;
    case pic_date:	edit_date (desc, picture, output); return 0;
    case pic_float:	edit_float (desc, picture, output); return 0;
    default:
#ifndef PYXIS
...
l = MOVQ_get_string (desc, &p, (VARY*)temp, (USHORT) sizeof (temp));
end = p + l;
