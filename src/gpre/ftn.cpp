...
//____________________________________________________________
//
//	$Id: ftn.cpp,v 1.1.1.1 2001-05-23 13:25:32 tamlin Exp $
//
...
static gen_database_data( ACT action)
{
	DBB db;
	TPB tpb;
	REQ request;
...
static gen_database_decls( ACT action)
{
	DBB db;
	REQ request;
	POR port;
...
static gen_drop_database( ACT action)
{
	DBB db;
...
static gen_request_data( REQ request)
{
	POR port;
	REF reference;
	BLB blob;
...
static gen_request_decls( REQ request)
{
	int length;
	REF reference;
	BLB blob;
...
static gen_return_value( ACT action)
{
	UPD update;
	REF reference;
	REQ request;
	...
	if (array_decl_list)
		for (this_array = array_decl_list; this_array;
			 ...
			 this_array->adl_next) if (field->fld_array_info->ary_ident ==
									   this_array->adl_gds_ident) return;
...
static make_ok_test( ACT action, REQ request)
{
...
static make_ready( DBB db, TEXT * filename, TEXT * vector, REQ request)
{
	TEXT s1[32], s2[32];
...
static printa( SCHAR * column, SCHAR * string, ...)
{
	va_list ptr;
	SCHAR s[256];
...
static status_and_stop( ACT action)
{
