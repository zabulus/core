...
int RESTORE_restore( TEXT * file_name, TEXT * database_name)
{
/**************************************
 *
...
/* Activate the indices for foreign keys and do another commit  */
	...
	if (tdgbl->global_trans)
	{
		...
		if (gds__status[0] == gds_arg_gds &&
			gds__status[1] == 0 &&
			gds__status[2] != gds_arg_end)
		{
...
/* Check to see if there is a warning */
	if (gds__status[0] == gds_arg_gds && gds__status[1] == 0
		&& gds__status[2] != gds_arg_end)
		BURP_print_warning(gds__status);
...
/* set sync writes to engine default */
	...
	*d++ = (UCHAR) tdgbl->hdr_forced_writes;	/* set forced writes to the value which was in the header */
	...
	if (isc_attach_database(tdgbl->status_vector, 0, GDS_VAL(database_name),
		...
		general_on_error();
	if (isc_detach_database
		(tdgbl->status_vector,
		 (isc_db_handle *) GDS_REF(db_handle))) general_on_error();
	...
	if (tdgbl->gbl_sw_mode == TRUE && tdgbl->gbl_sw_mode_val == TRUE) {
		...
		*d++ = TRUE;			/* set database to readOnly mode */
		...
		if (isc_attach_database
			(tdgbl->status_vector, 0, GDS_VAL(database_name),
			 (isc_db_handle *) GDS_REF(db_handle), l, dpb))
			general_on_error();
		if (isc_detach_database
			(tdgbl->status_vector, (isc_db_handle *) GDS_REF(db_handle)))
			general_on_error();
...
static void add_files( UCHAR * file_name)
{
/**************************************
 *
 *	a d d _ f i l e s
	...
	for (file = tdgbl->gbl_sw_files; file; file = file->fil_next) {
	   ...
	   END_STORE;
	    ...
	    END_ERROR;
			if (req_handle1)
				isc_release_request(req_status, &req_handle1);
			BURP_verbose(57, file->fil_name, (void *) start, NULL_PTR,
						 NULL_PTR, NULL_PTR);
			...
			BURP_print(96, (void *) file->fil_length, (void *) (start - 1),
 ...
 **************************************/
	...
	if (!tdgbl->gbl_sw_skip_count) {
		gds__msg_format(NULL_PTR, 12, type, sizeof(t_name), t_name, NULL_PTR,
						NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR);
		BURP_print(80, t_name, (void *) bad_attr, NULL_PTR, NULL_PTR,
				   NULL_PTR);
		...
		if (scan_next_attr == NO_SKIP) {
			...
			GET_SKIP(skip_count);
			BURP_print(203, (void *) skip_count, (void *) bad_attr, NULL_PTR,
					   NULL_PTR, NULL_PTR);
			/*msg 203: skipped %d bytes after reading a bad attribute %d */
		}
		else {
			skip_count++;
			BURP_print(205, (void *) skip_count, (void *) bad_attr, NULL_PTR,
					   NULL_PTR, NULL_PTR);
...
static void create_database( UCHAR * file_name)
{
/**************************************
 *
...
/* msg 32 Expected database description record */
	...
	if (tdgbl->gbl_sw_page_size && (tdgbl->gbl_sw_page_size < page_size)) {
		BURP_print(110, (void *) page_size, (void *) tdgbl->gbl_sw_page_size,
				   NULL_PTR, NULL_PTR, NULL_PTR);
...
/* start database up shut down */
	...
	*d++ = 0;
	...
	if (isc_create_database(status_vector,
							0,
							GDS_VAL(file_name),
							GDS_REF(tdgbl->db_handle), l, dpb, 0)) {
...
/* msg 33 failed to create database %s */
	...
	BURP_verbose(74, file_name, (void *) page_size, NULL_PTR, NULL_PTR,
				 NULL_PTR);
/* msg 74 created database %s, page_size %ld bytes */
}


static void decompress( UCHAR * buffer, USHORT length)
{
/**************************************
 *
 *	d e c o m p r e s s
 ...
 **************************************/
	...
	while (p < end) {
		...
		if (count > 0) {
			if (end - p < count) {
				BURP_print(202, (void *) count, (void *) (end - p), NULL_PTR,
						   NULL_PTR, NULL_PTR);
			...
			if (end + count < p) {
				BURP_print(202, (void *) count, (void *) (p - end), NULL_PTR,
						   NULL_PTR, NULL_PTR);
				/* msg 202: adjusting a decompression length error: invalid length %d was adjusted to %d */
				count = p - end;
			}
...
static REL find_relation( TEXT * name)
{
/**************************************
 *
...
static int get_acl(
				   TEXT * owner_nm,
				   ISC_QUAD * blob_id, ISC_QUAD * new_blob_id)
{
/**************************************
 *
 *	g e t _ a c l
...
*/
	...
	isc_get_segment(status_vector,
					GDS_REF(blob), GDS_REF(l), max_segment, GDS_VAL(buffer));
	...
	*to++ = owner_nm_len;
	...
	if (isc_put_segment(status_vector,
						GDS_REF(blob_handle), new_len, GDS_VAL(new_buffer))) {
		if (buffer != static_buffer)
			BURP_FREE(buffer);
		if (new_buffer != NULL_PTR)
			BURP_FREE(new_buffer);
...
static void get_array( REL relation, UCHAR * record_buffer)
{
/**************************************
 *
 *	g e t _ a r r a y
 ...
 **************************************/
	...
	SLONG *blob_id, return_length, slice_length, *range, *end_ranges, lcount;
...
/* Pick up attributes */
    ...
    **/
		...
		for (current_dim = 1; current_dim < field->fld_dimensions * 2;
			 current_dim += 2)
			ib_fprintf(ib_stderr, "%d ", fld_ranges[current_dim]);
		ib_fprintf(ib_stderr, "\nCalculated Last element upper bounds :\n");
		for (current_dim = 0; current_dim < field->fld_dimensions;
			 current_dim++)
			ib_fprintf(ib_stderr, "%d ", last_element_dim[current_dim]);
		...
		for (current_dim = 1; current_dim < field->fld_dimensions * 2;
			 current_dim += 2)
			ib_fprintf(ib_stderr, "%d ", field->fld_ranges[current_dim]);
		ib_fprintf(ib_stderr, "]");
#endif
		data_at = 0;
	/**
	...
	**/
			...
			if (data_at == 0) {
				...
				if (tdgbl->gbl_sw_transportable) {
					if (GET_ATTRIBUTE(attribute) != att_xdr_array)
						...
						lcount = xdr_buffer.lstr_length =
							xdr_buffer.lstr_allocated;
						xdr_buffer.lstr_address =
							BURP_ALLOC(xdr_buffer.lstr_allocated);
						xdr_slice.lstr_allocated = xdr_slice.lstr_length =
							return_length;
				...
				if (tdgbl->gbl_sw_transportable)
					CAN_slice(&xdr_buffer, &xdr_slice, FALSE, blr_length,
							  blr_buffer);
			}

			if (isc_put_slice(status_vector,
							  GDS_REF(tdgbl->db_handle),
							  GDS_REF(gds__trans), (ISC_QUAD *) GDS_VAL(blob_id), blr_length, blr_buffer, 0,	/* param length for subset of an array handling */
							  (ISC_LONG *) 0,	/* param for subset of an array handling */
							  elements_written * field->fld_length,
							  GDS_VAL(buffer + data_at))) {
				BURP_print(81, field->fld_name, NULL_PTR, NULL_PTR, NULL_PTR,
						   NULL_PTR);
	  ...
	  /** This is the regular case we've got the entire array **/
		...
		for (range = fld_ranges, count = 0; range < end_ranges;
			 range += 2, count++) {
			STUFF(gds__sdl_do2);
			STUFF(count);
			STUFF(gds__sdl_long_integer);
...
static void get_blob( FLD fields, UCHAR * record_buffer)
{
/**************************************
 *
 *	g e t _ b l o b
...
/* Create new blob */
	...
	if (isc_create_blob(status_vector,
						GDS_REF(tdgbl->db_handle),
						GDS_REF(gds__trans),
						GDS_REF(blob), (ISC_QUAD *) GDS_VAL(blob_id)))
		BURP_error_redirect(status_vector, 37, 0, 0);
	/* msg 37 gds__create_blob failed */
...
static void get_blr_blob( ISC_QUAD * blob_id, USHORT glb_trans)
{
/**************************************
 *
...
static REC_TYPE get_data( REL relation)
{
/**************************************
 *
 *	g e t _ d a t a
...
/* msg 27 gds__compile_request failed */
	...
	BURP_verbose(124, relation->rel_name, NULL_PTR, NULL_PTR, NULL_PTR,
				 NULL_PTR);
...
/* msg 55 Expected XDR record length */
				...
				if (l > data.lstr_allocated) {
					...
					data.lstr_address =
						(UCHAR *) BURP_ALLOC(data.lstr_allocated);
				}
...
/* msg 41 expected data attribute */
		...
		if ((records % RESTORE_VERBOSE_INTERVAL) == 0)
			BURP_verbose(107, (void *) records, NULL_PTR, NULL_PTR, NULL_PTR,
						 NULL_PTR);
 ...
 if (tdgbl->gbl_sw_incremental) {
					BURP_print(138, relation->rel_name, NULL_PTR, NULL_PTR,
							   NULL_PTR, NULL_PTR);
...
/* msg 47 warning -- record could not be restored */
				...
				BURP_print(114, relation->rel_name, NULL_PTR, NULL_PTR,
						   NULL_PTR, NULL_PTR);
...
/* msg 48 gds__send failed */
	...
	isc_release_request(status_vector, GDS_REF(request));
	if (tdgbl->gbl_sw_incremental) {
		BURP_verbose(72, relation->rel_name, NULL_PTR, NULL_PTR, NULL_PTR,
					 NULL_PTR);
...
static FLD get_field( REL relation)
{
/**************************************
 *
 *	g e t _ f i e l d
...
static void get_function_arg( GDS_NAME funcptr)
{
/**************************************
 *
...
static BOOLEAN get_index( REL relation)
{
/**************************************
 *
 *	g e t _ i n d e x
...
static void get_misc_blob(
						  ISC_QUAD * blob_id,
						  USHORT sub_type, USHORT glb_trans)
{
/**************************************
 *
...
/* Create new blob */
	...
	if (isc_create_blob2(status_vector,
						 ...
						 GDS_VAL(blob_id),
						 bpb_length,
						 bpb)) BURP_error_redirect(status_vector, 37, 0, 0);
	/* msg 37 gds__create_blob failed */
...
static BOOLEAN get_procedure_prm( GDS_NAME procptr)
{
/**************************************
 *
...
/* Eat up misc. records */
	...
	while (GET_RECORD(record) != rec_data)
		...
		case rec_relation_end:
			if (tdgbl->gbl_sw_incremental) {
				BURP_verbose(170, relation->rel_name, NULL_PTR, NULL_PTR,
							 NULL_PTR, NULL_PTR);
...
static void get_source_blob( ISC_QUAD * blob_id, USHORT glb_trans)
{
/**************************************
 *
...
static USHORT get_text( TEXT * text, ULONG length)
{
/**************************************
 *
 *	g e t _ t e x t
...
static BOOLEAN get_trigger_old( REL relation)
{
/**************************************
 *
	...
	case obj_procedure:
		{
			...
			for (proc = tdgbl->procedures; proc; proc = proc->prc_next)
				if (!strcmp(proc->prc_name, relation_name)) {
					exists = 1;
					local_trans =
						tdgbl->global_trans ? tdgbl->
						global_trans : gds__trans;
					break;
				}
		}
		break;
...
static BOOLEAN get_view( REL relation)
{
/**************************************
 *
 *	g e t _ v i e w
...
static void ignore_array( REL relation)
{
/**************************************
 *
...
static REC_TYPE ignore_data( REL relation)
{
/**************************************
 *
...
/* msg 41 expected data attribute */
	...
	BURP_verbose(106, (void *) records, NULL_PTR, NULL_PTR, NULL_PTR,
				 NULL_PTR);
...
static void realign( UCHAR * buffer, REL relation)
{
/**************************************
 *
 *	r e a l i g n
...
static USHORT recompute_length( REL relation)
{
/**************************************
 *
...
static BOOLEAN restore( TEXT * file_name, TEXT * database_name)
{
/**************************************
 *
 *	r e s t o r e
...
/* restore only from those backup files created by current or previous GBAK */
	...
	if (tdgbl->RESTORE_format < 1
		|| tdgbl->RESTORE_format > ATT_BACKUP_FORMAT)
#ifdef SUPERSERVER
 ...
 */
	...
	EXEC SQL SET TRANSACTION NAME tdgbl->
		global_trans ISOLATION LEVEL READ COMMITTED;
	...
	while (GET_ATTRIBUTE(attribute) != att_end)
		...
		case att_database_security_class:
			...
			get_text(tdgbl->database_security_class,
					 sizeof(tdgbl->database_security_class));
			break;
...
/* Get global fields and relations */
	...
	while (GET_RECORD(record) != rec_end)
		...
		case rec_relation_data:
			if (flag) {
				BURP_verbose(68, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR,
							 NULL_PTR);
				/* msg 68 committing meta data */
				COMMIT;
	       ON_ERROR
...
static void restore_security_class( TEXT * owner_nm, TEXT * sec_class_nm)
{
/**************************************
 *
...
static void store_blr_gen_id( GDS_NAME gen_name, SINT64 value)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (!value) {
		BURP_verbose(185, gen_name, (void *) value, NULL_PTR, NULL_PTR,
					 NULL_PTR);
...
/* msg 42 Failed in store_blr_gen_id */
	...
	if (isc_start_request(status_vector, GDS_REF(gen_id_reqh), GDS_REF(gds__trans),	/* use the same one generated by gpre */
						  0)) {
		isc_print_blr(blr_buffer, NULL_PTR, NULL_PTR, 0);
		BURP_error_redirect(status_vector, 42, 0, 0);
/* msg 42 Failed in store_blr_gen_id */
...
static void stuff_string( SCHAR ** ptr, TEXT * string)
{
/**************************************
 *
...
static BOOLEAN bug_8183( TGBL tdgbl)
{
/**************************************
 *
 *      b u g _ 8 1 8 3
