...
void PRO_close( DBB database, void *blob)
{
/**************************************
 *
 *	P R O _ c l o s e
...
void *PRO_fetch_procedure( DBB database, TEXT * proc)
{
/**************************************
 *
...
int PRO_get_line( void *blob, TEXT * buffer, USHORT size)
{
/**************************************
 *
...
void *PRO_open_blob( DBB database, SLONG * blob_id)
{
/**************************************
 *
...
/* create blob parameter block since procedure is a text blob */
	...
	*p++ = QLI_interp >> 8;
	...
	if (gds__open_blob2(status_vector,
						...
						GDS_VAL(blob_id),
						bpb_length,
						bpb)) ERRQ_database_error(database, status_vector);
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_scan_blobs) X IN DB.QLI$PROCEDURES
        ...
        (*routine) (arg, X.QLI$PROCEDURE_NAME,
					strlen(X.QLI$PROCEDURE_NAME), database, &X.QLI$PROCEDURE);
	END_FOR
}
...
void *PRO_transaction( DBB database, int update_flag)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (gds__ddl(gds__status,
				 GDS_REF(DB),
				 GDS_REF(gds__trans), sizeof(dyn_gdl1), dyn_gdl1)) {
		PRO_rollback(dbb);
		IBERROR(73);
	...
	if (gds__ddl(gds__status,
				 GDS_REF(DB),
				 GDS_REF(gds__trans), sizeof(dyn_gdl2), dyn_gdl2)) {
		PRO_rollback(dbb);
		IBERROR(74);
	...
	if (gds__ddl(gds__status,
				 GDS_REF(DB),
				 GDS_REF(gds__trans), sizeof(dyn_gdl3), dyn_gdl3)) {
		PRO_rollback(dbb);
		IBERROR(75);
	...
	if (gds__ddl(gds__status,
				 GDS_REF(DB),
				 GDS_REF(gds__trans), sizeof(dyn_gdl4), dyn_gdl4)) {
		PRO_rollback(dbb);
		IBERROR(409);
