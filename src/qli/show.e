...
static CONST UCHAR
  ...
  text_bpb[] = { gds__bpb_version1, gds__bpb_target_interp, 2,
...
gds__bpb_version1, gds__bpb_source_type, 1, 3,
		...
		gds__bpb_target_type, 1, 1}, blr_bpb[] = {
gds__bpb_version1, gds__bpb_source_type, 1, 2, gds__bpb_target_type, 1, 1};
 ...
 **************************************/
	...
	if (source[0] || source[1])
		show_text_blob(database, "\t", source_msg, source, 0, NULL_PTR,
					   FALSE);
	else if (blr[0] || blr[1]) {
		if (blr_msg)
...
static show_filter_detail( DBB database, TEXT * filter_name)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_show_filter_detail])
		...
		show_text_blob(database, "\t", 448, &F.RDB$DESCRIPTION, 0, NULL_PTR,
					   FALSE);
	END_FOR;
	return count;
}
...
static show_func_detail( DBB database, TEXT * func_name)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_show_func_detail])
		...
		show_text_blob(database, "\t", 421, &F.RDB$DESCRIPTION, 0, NULL_PTR,
					   FALSE);
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_show_global_field])
		...
		show_text_blob(database, "\t", 277, &F.RDB$DESCRIPTION, 0, NULL_PTR,
					   FALSE);
		...
		show_text_blob(database, "\t", 283, &F.RDB$QUERY_HEADER, 0, NULL_PTR,
					   FALSE);
	END_FOR;
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_show_global_fields])
		...
		show_datatype(database, MET_get_datatype(F.RDB$FIELD_TYPE),
					  ...
					  F.RDB$FIELD_SUB_TYPE, F.RDB$SEGMENT_LENGTH,
					  MET_dimensions(database, F.RDB$FIELD_NAME));
		show_text_blob(database, "\t", 262, &F.RDB$DESCRIPTION, 0, NULL_PTR,
					   FALSE);
		ib_printf("\n");
	END_FOR;
 ...
 **************************************/
	...
	if (QLI_matching_language) {
		strncpy(buffer, QLI_matching_language->con_data,
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_show_sys_triggers])
        ...
        if (!X.RDB$TRIGGER_BLR.NULL) {
			...
			show_blob_info(&X.RDB$TRIGGER_BLR, &X.RDB$TRIGGER_SOURCE, 377,	/* Msg377 Source for the trigger is not available.  Trigger BLR: */
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_show_view])
        ...
        if (X.RDB$VIEW_SOURCE.NULL) {
			...
			show_text_blob(database, "\t", 0, &X.RDB$VIEW_SOURCE, 0, NULL_PTR,
						   FALSE);
		}
	END_FOR;
}
