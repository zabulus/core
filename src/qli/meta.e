...
static CONST UCHAR db_log_info[] =
	{ gds__info_logfile, gds__info_cur_logfile_name,
		gds__info_cur_log_part_offset };
...
int MET_declare( DBB database, FLD variable, NAM field_name)
{
/**************************************
 *
...
void MET_define_field( DBB database, FLD field)
{
/**************************************
 *
...
*/
	...
	for (ptr = (NAM *) fields->syn_arg, position = 0;
		...
		for (present = FALSE, field = relation->rel_fields; field;
			 field =
			 field->
			 fld_next)
		...
		if (!present) {
			...
			ERRQ_print_error(236, (*ptr)->nam_string,
							 relation->rel_symbol->sym_string, NULL, NULL, NULL);	/* Msg236 Field %s does not occur in relation %s */
		}
...
void MET_define_relation( REL relation, REL source)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (source)
		...
		for (field = relation->rel_fields, position = 1; field;
...
void MET_define_sql_relation( REL relation)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR(REQUEST_HANDLE database->dbb_requests[REQ_check_fld])
		RFR IN DB.RDB$RELATION_FIELDS WITH RFR.RDB$FIELD_SOURCE EQ name->
			nam_string REDUCED TO RFR.RDB$RELATION_NAME 
		if (!count)
...
void MET_delete_relation( REL relation)
{
/**************************************
 *
...
#ifdef DEV_BUILD
void MET_index_info(
					SCHAR * relation_name, SCHAR * index_name, SCHAR * buffer)
{
/**************************************
 *
...
SLONG *MET_meta_transaction(DBB database, int update_flag)
{
/**************************************
 *
...
void MET_modify_field( DBB database, FLD field)
{
/**************************************
 *
...
void MET_modify_relation( REL relation, FLD fields)
{
/**************************************
 *
	...
	for (ptr = node->syn_arg; ptr < end; ptr++) {
		...
		if (create_flag)
			gds__create_database(status_vector,
								 ...
								 GDS_VAL(dbb->dbb_filename),
								 GDS_REF(dbb->dbb_handle),
								 dpb_length, GDS_VAL(dpb), 0);
		else
			gds__attach_database(status_vector,
								 0,
								 GDS_VAL(dbb->dbb_filename),
								 GDS_REF(dbb->dbb_handle),
								 dpb_length, GDS_VAL(dpb));
		if (status_vector[1])
			break;
	}
...
void MET_sql_alter_table( REL relation, FLD fields)
{
/**************************************
 *
...
SLONG *MET_transaction( NOD_T node_type, DBB database)
{
/**************************************
 *
...
static void add_field( REL relation, FLD field, USHORT position)
{
/**************************************
 *
 *	a d d _ f i e l d
...
static void add_sql_field( REL relation, FLD field, USHORT position, RLB rlb)
{
/**************************************
 *
...
static int blob_copy( RLB rlb, REL source, SLONG * source_blob_id)
{
/**************************************
 *
 *	b l o b _ c o p y
 ...
 **************************************/
	...
	if (gds__open_blob(status_vector,
					   ...
					   GDS_REF(source_blob), GDS_VAL(source_blob_id))) {
		rollback_update(DB);
...
static void change_field( REL relation, FLD field)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (global_field
		&&
		!(check_global_field(database, NULL_PTR, global_field->sym_string))) {
		rollback_update(database);
...
static int check_global_field( DBB database, FLD field, TEXT * name)
{
/**************************************
 *
...
static int check_relation( REL relation)
{
/**************************************
 *
...
static int clone_fields( REL target, REL source)
{
/**************************************
 *
...
static clone_global_fields( REL target, REL source)
{
/**************************************
 *
...
static void define_global_field( DBB database, FLD field, SYM name)
{
/**************************************
 *
...
static void delete_fields( REL relation)
{
/**************************************
 *
...
static STATUS detach( STATUS * status_vector, DBB dbb)
{
/**************************************
 *
 *	d e t a c h
 ...
 **************************************/
	...
	gds__database_info(status_vector,
					   ...
					   sizeof(db_info), db_info, sizeof(buffer), buffer);
...
/* loop up to 10 times to allow the file to be archived */
	...
	while (TRUE) {
		...
		if (WALF_get_linked_logs_info
			(gds__status, db_name, cur_log, part_offset, &log_count, next_log,
			 &next_offset, &last_log_flag, &not_archived) != SUCCESS)
			ERRQ_database_error(dbb, gds__status);
 ...
 **************************************/
	...
	*p2++ = QLI_interp >> 8;
	...
	if (gds__open_blob2(status_vector,
						GDS_REF(database->dbb_handle),
						GDS_REF(gds__trans),
						GDS_REF(blob), GDS_VAL(blob_id), bpb_length, bpb))
#else
	if (gds__open_blob(status_vector,
					   GDS_REF(database->dbb_handle),
...
/* Get all relations in database.  For each relation make up a
   ...
   into the hash table. */
	...
	FOR(REQUEST_HANDLE request) X IN DB.RDB$RELATIONS SORTED BY DESCENDING
		X.RDB$RELATION_NAME relation = (REL) ALLOCP(type_rel);
		...
		case blr_text2:
			...
			if (QLI_interp == scale_specified_in_blr)
				...
				if (KANJI_euc_len(blr, length_specified_in_blr, &length)) {
					ERRQ_print_error(258, (TEXT *) op, NULL, NULL, NULL,
									 NULL);
			...
			if ((scale_specified_in_blr == gds__interp_eng_ascii) ||
				...
				while (l--)
					*p++ = BLR_BYTE;
					 ...
					 scale_specified_in_blr == gds__interp_jpn_sjis) {
				if (KANJI_sjis2euc
					(blr, length_specified_in_blr, p, length, &length)) {
					ERRQ_print_error(258, (TEXT *) op, NULL, NULL, NULL,
									 NULL);
					/* Msg258 don't understand BLR operator %d */
				}
 ...
 **************************************/
	...
	if (gds__open_blob(status_vector,
					   GDS_REF(DB),
					   GDS_REF(gds__trans),
					   GDS_REF(handle), GDS_VAL(blob_id))) return NULL;
	...
	for (;;) {
		...
		if (gds__get_segment(status_vector,
							 GDS_REF(handle),
							 GDS_REF(length), length, GDS_VAL(ptr))) break;
		ptr += length;
	}
...
static void purge_relation(REL relation)
{
/**************************************
 *
...
static void put_dyn_string( RLB rlb, TEXT * string)
{
/**************************************
 *
...
/*      For each specified field create a separate grant string */
	...
	for (name = (NAM *) names->syn_arg, end = name + names->syn_count;
		 name < end; name++)
		if (fields->syn_count) {
			for (field = (NAM *) fields->syn_arg, end_field =
				 field + fields->syn_count; field < end_field; field++)
				stuff_priv(rlb, type, relation_name, privileges,
						   (*name)->nam_string, (*field)->nam_string);
		}
		else
			stuff_priv(rlb, type, relation_name, privileges,
					   (*name)->nam_string, NULL_PTR);
...
static void stuff_priv(
					   ...
					   USHORT type,
					   TEXT * relation,
					   USHORT privileges, TEXT * user, TEXT * field)
{
/**************************************
 *
 *	s t u f f _ p r i v
