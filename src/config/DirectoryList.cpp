...
DirectoryList::DirectoryList(const char* options)
{
	...
	const Firebird::string option(options, p - options);
...
bool DirectoryList::validateFilename(const char* fileName) const
{
	if (!enabled)
		return false;
...
bool DirectoryList::matchFileName(const char* directory, const char* fileName) const
{
#pragma FB_COMPILER_MESSAGE("Missing functionality here")
	return true;
}
