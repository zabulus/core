 ...
 */
/*
  $Id: isql.e,v 1.1.1.1 2001-05-23 13:26:03 tamlin Exp $
...
static UCHAR blr_bpb[] = { isc_bpb_version1,
	...
	isc_bpb_version1,
		...
		isc_bpb_target_type, 1, BLOB_text}, acl_bpb[] = {
	isc_bpb_version1,
...
/* Init the diagnostics and help files */
	...
	case ERR:
		gds__msg_format(NULL_PTR, ISQL_MSG_FAC, USAGE1, sizeof(helpstring),
						helpstring, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR,
						NULL_PTR);
...
/* Transaction for all frontend commands */
	if (DB && !gds__trans)
		if (isc_start_transaction
			(isc_status, &gds__trans, 1, &DB, 0, (SCHAR *) 0)) {
	...
	if (!Db_name[0]) {
		if (!Quiet) {
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, NO_DB, sizeof(errbuf),
							errbuf, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR,
							NULL_PTR);
			STDERROUT(errbuf, 1);
#ifdef GUI_TOOLS
			/*
...
void ISQL_errmsg( STATUS * status)
{
/**************************************
 *
...
#define TRANSLATE_CP
	...
	if (Quiet)
		...
		gds__msg_format(NULL_PTR, ISQL_MSG_FAC, 0, sizeof(errbuf), errbuf,
						(TEXT *) isc_sqlcode(status), NULL_PTR, NULL_PTR,
						NULL_PTR, NULL_PTR);
#ifndef GUI_TOOLS
		TRANSLATE_CP;
		STDERROUT(errbuf, 1);
...
void ISQL_warning( STATUS * status)
{
/**************************************
 *
...
int ISQL_frontend_command(
						  TEXT * string,
						  IB_FILE * ipf, IB_FILE * opf, IB_FILE * sf)
{
/**************************************
 *
		...
		if (endpassword) {
			...
			*endpassword = NULL;
			...
			ib_fprintf(sf, "%s %s %s%s",
					   string, WISQL_PASSWORD_COMMENT_LITERAL, Term, NEWLINE);
			ib_fflush(sf);
...
int ISQL_extract(
				 SCHAR * string,
				 int type, IB_FILE * ipf, IB_FILE * opf, IB_FILE * sf)
{
/**************************************
 *
...
/* Save off the transaction handle so we can set it to NULL */
	...
	if (type == EXTRACT_VIEW) {
		...
		if (type == EXTRACT_TABLE) {
			ib_fprintf(sf, "/* Extract Table %s%s */%s", string, Term,
					   NEWLINE);
			ib_fflush(sf);
			...
			ib_fprintf(sf, "/* Extract Database %s%s */%s", Db_name, Term,
					   NEWLINE);
			ib_fflush(sf);
...
/* Set up the flags for Windows ISQL */
	...
	else if (*db_objects & VIEW_OBJECTS)
		*db_objects ^= VIEW_OBJECTS;
	return;
}
#endif
...
#ifdef GUI_TOOLS
SSHORT ISQL_init(IB_FILE * inputfile, IB_FILE * outputfile)
{
/**************************************
 *
 *	I S Q L _ i n i t
...
void ISQL_print_validation(IB_FILE * fp,
						   ISC_QUAD * blobid, SSHORT flag, SLONG * trans)
{
/**************************************
 *
...
#ifdef GUI_TOOLS
int ISQL_sql_statement(TEXT * string,
					   IB_FILE * ipf, IB_FILE * opf, IB_FILE * sf)
{
/**************************************
 *
...
*/
	...
	gds__msg_format(NULL_PTR, ISQL_MSG_FAC, ADD_PROMPT, sizeof(infobuf),
					infobuf, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR,
					NULL_PTR);
	STDERROUT(infobuf, 1);
				...
				*ivar->sqlind = -1;
				...
				case SQL_TYPE_TIME:
					if (3 != sscanf(buffer, "%d:%d:%d", &times.tm_hour,
						...
						ISQL_msg_get(TIME_ERR, msg, buffer, NULL, NULL, NULL,
									 NULL);
				...
				case SQL_TYPE_DATE:
					if (3 != sscanf(buffer, "%d/%d/%d", &times.tm_mon,
						...
						ISQL_msg_get(DATE_ERR, msg, buffer, NULL, NULL, NULL,
									 NULL);
						STDERROUT(msg, 1);	/* Bad date %s\n */
						done = TRUE;
					}
   ...
   gds__temp_file returns -1 on error, not 0] */
	...
	if (EXTRACT_list_table(source, destination, domain_flag, -1)) {
		gds__msg_format(NULL_PTR, ISQL_MSG_FAC, NOT_FOUND, sizeof(errbuf),
						errbuf, source, NULL_PTR, NULL_PTR, NULL_PTR,
						NULL_PTR);
		STDERROUT(errbuf, 1);
		(void) ib_fclose(Out);
	}
		...
		if (system(cmd)) {
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, COPY_ERR, sizeof(errbuf),
							errbuf, destination, altdb, NULL_PTR, NULL_PTR,
							NULL_PTR);
			STDERROUT(errbuf, 1);
		}
	}
   ...
   gds__temp_file returns -1 on error, not 0] */
	...
	if (Ofp == (IB_FILE *) - 1) {
		...
		gds__msg_format(NULL_PTR, ISQL_MSG_FAC, FILE_OPEN_ERR, MSG_LENGTH,
						errbuf, Tmpfile, NULL_PTR, NULL_PTR, NULL_PTR,
						NULL_PTR);
		STDERROUT(errbuf, 1);
		Exit_value = FINI_ERROR;
		exit(Exit_value);
		...
		if (!Db_name[0] && (ret == CONT)) {
			if (!Quiet) {
				gds__msg_format(NULL_PTR, ISQL_MSG_FAC, NO_DB, MSG_LENGTH,
								errbuf, NULL_PTR, NULL_PTR, NULL_PTR,
								NULL_PTR, NULL_PTR);
				STDERROUT(errbuf, 1);
#ifdef GUI_TOOLS
				/*
...
/* If there is a file name specified, try to open it */
	...
	if (*file) {
		...
		if (fp = ib_fopen(path, "r")) {
			...
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, FILE_OPEN_ERR, MSG_LENGTH,
							errbuf, file, NULL_PTR, NULL_PTR, NULL_PTR,
							NULL_PTR);
			STDERROUT(errbuf, 1);
		}
	}
	...
	if (M__trans) {
		if (Interactive) {
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, COMMIT_PROMPT,
							sizeof(infobuf), infobuf, NULL_PTR, NULL_PTR,
							NULL_PTR, NULL_PTR, NULL_PTR);
			ISQL_prompt(infobuf);
	...
	while (*p == '/' && *(p + 1) == '*') {
		...
		if (*p)
			...
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, CMD_ERR, MSG_LENGTH,
							errbuf, statement, NULL_PTR, NULL_PTR, NULL_PTR,
							NULL_PTR);
			STDERROUT(errbuf, 1);
			ISQL_FREE(errbuf);
			ISQL_FREE(buffer);
...
/* Free the frontend command */
	...
	if (ret == ERR) {
		if (bad_dialect == TRUE)
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, CMD_ERR, MSG_LENGTH,
							errbuf, bad_dialect_buf, NULL_PTR, NULL_PTR,
							NULL_PTR, NULL_PTR);
		else
			gds__msg_format(NULL_PTR, ISQL_MSG_FAC, CMD_ERR, MSG_LENGTH,
							errbuf, cmd, NULL_PTR, NULL_PTR, NULL_PTR,
							NULL_PTR);
		STDERROUT(errbuf, 1);
	}
#endif
	...
	if (Cols) {
		...
		while (p) {
			sprintf(Print_buffer, "%s%s width: %d%s",
					TAB, p->col_name, p->col_len, NEWLINE);
			ISQL_printf(Out, Print_buffer);
			p = p->collist_next;
		}
	}
...
static SSHORT newdb(
					TEXT * dbname,
					TEXT * usr,
					TEXT * psw, TEXT * numbufs, TEXT * sql_role_nm)
{
/**************************************
 *
 *	n e w d b
	...
	while (i < argc) {
		...
		if (*s == switchchar) {
			...
			/* Get the WISQL specific session file */
				...
				command line option '-s' is reserved for '-s[qldialect] n' Bie
					Tie
				...
				gds__msg_format(NULL_PTR, ISQL_MSG_FAC, VERSION, MSG_LENGTH,
								errbuf, GDS_VERSION, NULL_PTR, NULL_PTR,
								NULL_PTR, NULL_PTR);
				...
				gds__msg_format(NULL_PTR, ISQL_MSG_FAC, SWITCH, MSG_LENGTH,
								errbuf, s + 1, NULL_PTR, NULL_PTR, NULL_PTR,
								NULL_PTR);
				STDERROUT(errbuf, 1);
				ret = ERR;
			}
...
/* The printlength should have in it the right length */
	...
	if ((var->sqltype & 1) && (*var->sqlind < 0)) {
		...
		case SQL_ARRAY:
			...
			sprintf(blobbuf, "%x:%x", blobid.gds_quad_high,
					blobid.gds_quad_low);
		...
		case SQL_TIMESTAMP:
			...
			if (SQL_dialect > SQL_DIALECT_V5) {
				sprintf(d, "%4.4d-%2.2d-%2.2d %2.2d:%2.2d:%2.2d.%4.4d",
				...
				if (Time_display)
					sprintf(d, "%2d-%s-%4d %2.2d:%2.2d:%2.2d.%4.4d",
...
static int print_item_blob( IB_FILE * fp, XSQLVAR * var, void *trans)
{
/**************************************
 *
...
/* Don't bother with null blobs */
	...
	if ((var->sqlsubtype != Doblob) && (Doblob != ALL_BLOBS)) {
		ISQL_msg_get(BLOB_SUBTYPE, msg, (TEXT *) Doblob,
					 (TEXT *) var->sqlsubtype, NULL, NULL, NULL);
	...
	case BLOB_text:
		...
		if (!isc_blob_lookup_desc
			(isc_status, &DB, &trans, var->relname, var->sqlname, &from_desc,
			 NULL)) {
...
/* Find out what kind of statement this is */
	...
	if (isc_dsql_sql_info(isc_status, &Stmt, sizeof(sqlda_info), sqlda_info,
		...
		ISQL_errmsg(isc_status);
	else if (info_buffer[0] == isc_info_sql_stmt_type) {
