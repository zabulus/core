...
void SHOW_dbb_parameters(SLONG * db_handle,
						 SCHAR * info_buf,
						 SCHAR * db_items,
 ...
 **************************************/
	...
	if (isc_database_info(status_vector,
						  ...
						  db_items,
						  BUFFER_LENGTH128,
						  buffer)) ISQL_errmsg(status_vector);
 ...
 */
		...
		case isc_info_reads:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of reads = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_writes:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of writes = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_fetches:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of fetches = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_marks:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of marks = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_num_buffers:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of buffers = %ld%s", value_out, NEWLINE);
			break;

		case isc_info_wal_num_io:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Wal number of io = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_wal_avg_io_size:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Wal average io size = %ld %s", value_out, NEWLINE);
			break;
		...
		case isc_info_current_memory:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Current memory size = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_max_memory:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Max memory size = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_attachment_id:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "DB attachment id = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_ods_version:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "ODS version = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_ods_minor_version:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Minor ODS version = %ld %s", value_out, NEWLINE);
			break;
		...
		case isc_info_update_count:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of updates = %ld %s", value_out, NEWLINE);
			break;

		case isc_info_insert_count:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of inserts = %ld %s", value_out NEWLINE);
			break;

		case isc_info_delete_count:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Number of deletes = %ld %s", value_out, NEWLINE);
			break;
		...
		case isc_info_version:
			value_out = isc_vax_integer(d, length);
			sprintf(info, "Info version number = %ld %s", value_out, NEWLINE);
			break;
...
int SHOW_grants( SCHAR * object, SCHAR * terminator)
{
/**************************************
 *
...
#ifdef GUI_TOOLS
void SHOW_query_database( int *tables, int *views, int *indices)
{
/**************************************
 *
...
#ifdef GUI_TOOLS
void SHOW_build_table_namelist( TBLLIST ** tbl_list)
{
/**************************************
 *
...
#ifdef GUI_TOOLS
void SHOW_build_view_namelist( VIEWLIST ** view_list)
{
/**************************************
 *
 ...
 **************************************/
	...
	TEXT query[100], gen_name[MAX_NAME_LENGTH * 2], *p1, *q1;
