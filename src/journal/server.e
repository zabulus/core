...
CMDS commands[] = {
	...
	"ERASE <db id>		", cmd_erase, 165,
	"\tdrop all files for non active database",
	"EXIT			", cmd_exit, 166, "\texit journal console program",
	"HELP			", cmd_help, 167, "\tdisplay this text",
	"RESET DELETE <db name>	", cmd_reset, 168,
...
int SERVER_start_server(int argc, SCHAR ** argv)
{
/**************************************
 *
...
static void add_backup_entry(DJB dbb, SLONG pid, SLONG fid)
{
/**************************************
 *
...
static CNCT alloc_connect(HANDLE handle, USHORT channel)
{
/**************************************
 *
...
static void backup_wal(
					   ...
					   SCHAR * file_name,
					   SLONG file_id,
					   SLONG file_seq, SLONG file_size, SLONG p_offset)
{
/**************************************
 *
 *	b a c k u p _ w a l
...
static void build_backup_array(DJB dbb)
{
/**************************************
 *
...
static void build_indexed_name(SCHAR * filename,
							   SCHAR * basename, SLONG seqno)
{
/**************************************
 *
...
#ifdef BSD_SOCKETS
static void close_mailbox(int slot)
{
/**************************************
 *
...
static void delete_backup_entry(DJB dbb, SLONG fid)
{
/**************************************
 *
...
static void delete_connection(CNCT connection)
{
/**************************************
 *
...
/* Disconnect from database */
	...
	if (database = connection->cnct_database) {
		...
		for (ptr = &database->djb_connections; *ptr;
			 ptr = &(*ptr)->cnct_db_next) if (*ptr == connection) {
...
/* Unlink from system */
	...
	if ((database) &&
		...
		(database->djb_use_count == 0)) delete_database(database);
}


static void delete_database(DJB database)
{
/**************************************
 *
...
static void delete_history_files(
								 CNCT connection,
								 SCHAR * dbname, SLONG db_id, SLONG last_dump)
{
/**************************************
 *
 ...
 **************************************/
	...
	for (database = databases; database; database = database->djb_next) {
        ...
        FOR J IN JOURNAL_FILES WITH J.DB_ID EQ database->djb_id
            ...
            WALF_set_log_header_flag(status, database->djb_db_name,
...
static void disable(CNCT connection, LTJC * msg)
{
/**************************************
 *
 *	d i s a b l e
...
#ifdef BSD_SOCKETS
static void disconnect(CNCT connection)
{
/**************************************
 *
...
static void enable(CNCT connection, LTJC * msg)
{
/**************************************
 *
 *	e n a b l e
...
static void free_database_entry(DJB database)
{
/**************************************
 *
...
#ifdef BSD_SOCKETS
static void get_message(JRNH * buffer, int length)
{
/**************************************
 *
 ...
 **************************************/
	...
	*wait_events++ = pending_connections[slot_journal].cnct_overlapped.hEvent;
	for (connection = connections; connection;
		 connection = connection->cnct_next) *wait_events++ =
			connection->cnct_overlapped.hEvent;

	wait_count = wait_events - wait_vec;
	if ((n = WaitForMultipleObjects(wait_count, wait_vec, FALSE, INFINITE)) <
		0) error(GetLastError(), "WaitForMultipleObjects");
	...
	if (!ReadFile(connection->cnct_handle, p, length, &len,
		...
		if (GetLastError() != ERROR_IO_PENDING)
			error(GetLastError(), "ReadFile");
									  ...
									  &connection->cnct_overlapped, &len,
...
#ifdef BSD_SOCKETS
static void init_handles(fd_set * mask_ptr)
{
/**************************************
 *
...
/* set mask for each accepted connections */
	...
	for (connection = connections; connection;
		 connection =
		 connection->cnct_next) SET_BIT(connection->cnct_handle, mask_ptr);
...
static DJB lookup_database(SCHAR * string, USHORT id, SSHORT find_disabled)
{
/**************************************
 *
...
static DJB make_database(CNCT connection, SCHAR * db_name, SCHAR * dir_name)
{
/**************************************
 *
...
#ifdef BSD_SOCKETS
static void open_mailbox(SCHAR * filename, USHORT slot)
{
/**************************************
 *
 ...
 **************************************/
	...
	*p = 0;
	...
	if (n > wait_vec_length) {
		...
		wait_vec_length += 10;
		wait_vec = (HANDLE *)
			MISC_alloc_jrnl((SLONG) (wait_vec_length * sizeof(HANDLE)));
	}
}
#endif
...
#ifdef BSD_SOCKETS
static void post_address(USHORT slot, struct sockaddr_in address)
{
/**************************************
 *
...
static void process_command(CNCT connection, TEXT * string)
{
/**************************************
 *
 ...
 **************************************/
	...
	case cmd_help:
		...
		for (command = commands; p = command->cmds_string; command++)
			if (command->cmds_msg)
				put_line(connection, command->cmds_msg,
						 (TEXT *) command->cmds_string, 0, 0);
		break;
...
static void process_archive_begin(CNCT connection, LTJA * msg)
{
/**************************************
 *
...
static void process_archive_end(CNCT connection, LTJA * msg)
{
/**************************************
 *
...
/* Handle situation where the database is already disabled */
	...
	if (database) {
	    ...
	    FOR D IN DATABASES WITH D.DB_ID EQ db_id strcpy(db, D.DB_NAME);
...
static void process_archive_error(CNCT connection, LTJA * msg)
{
/**************************************
 *
...
static void process_control_point(CNCT connection, LTJW * msg)
{
/**************************************
 *
...
static void process_disable_database(CNCT connection, TEXT * dbname)
{
/**************************************
 *
...
static void process_drop_database(CNCT connection, SCHAR * dbname)
{
/**************************************
 *
...
static void process_dump_file(CNCT connection, LTJW * msg)
{
/**************************************
 *
...
static void process_end_dump(CNCT connection, LTJW * msg)
{
/**************************************
 *
...
static void process_erase_database(CNCT connection, USHORT db_id)
{
/**************************************
 *
...
static void process_message(CNCT connection, JRNH * msg, USHORT length)
{
/**************************************
 *
...
static void process_reset_database(CNCT connection, SCHAR * dbname)
{
/**************************************
 *
...
static void process_restart_archive(CNCT connection, SCHAR * dbname)
{
/**************************************
 *
...
static void process_set_database(CNCT connection, SCHAR * dbname)
{
/**************************************
 *
...
static void process_start_dump(CNCT connection, LTJW * msg)
{
/**************************************
 *
 ...
 **************************************/
    ...
    FOR D IN DATABASES WITH D.DB_ID EQ connection->cnct_database->djb_id
        STORE X IN ONLINE_DUMP X.DB_ID = D.DB_ID;
...
static void process_wal_file(CNCT connection, LTJW * msg)
{
/**************************************
 *
 ...
 **************************************/
	...
	FOR D IN DATABASES WITH D.DB_ID EQ connection->cnct_database->djb_id
	   ...
	   /* Handle close file record for files without open */
        ...
        if (repeat == FALSE) {
            STORE X IN JOURNAL_FILES X.DB_ID = D.DB_ID;
...
/* restart failed backups */
	...
	if (database->djb_flags & DJB_archive) {
		if (msg->ltjw_mode == JRNW_CLOSE) {
			...
			backup_wal(database, msg->ltjw_data, file_id, file_seq, file_size,
					   p_offset);
		}
	}
...
static void put_line(
					 CNCT connection,
					 SSHORT msg, TEXT * arg1, TEXT * arg2, TEXT * arg3)
{
/**************************************
 *
 *	p u t _ l i n e
...
static void put_message(SSHORT msg_num, DJB database)
{
/**************************************
 *
 ...
 **************************************/
	...
	fprintf(log, "%.2d:%.2d:%.2d %.2d/%.2d/%.2d %s",
			...
			times.tm_mon + 1, times.tm_mday, times.tm_year, message);

	if (database)
		fprintf(log, " for \"%s\" (%d)\n", database->djb_db_name,
				database->djb_id);
	else
		fprintf(log, "\n");
...
#ifdef WIN_NT
static void read_asynchronous(CNCT connection)
{
/**************************************
 *
 ...
 **************************************/
	...
	if (ReadFile(connection->cnct_handle, connection->cnct_data,
				 sizeof(connection->cnct_data), &len,
				 &connection->cnct_overlapped)) {
		connection->cnct_read_length = len;
...
static void report_status(CNCT connection)
{
/**************************************
 *
...
static void restart_all_backups(int count)
{
/**************************************
 *
...
static void restart_backup(DJB database, int count)
{
/**************************************
 *
 ...
 **************************************/
	...
	for (i = 0; ((i < MAX_ARCHIVE) && (count)); i++) {
		...
		if (!ISC_check_process_existence
			(database->djb_bfi[i].bi_pid, 0, TRUE)) {
...
#ifdef BSD_SOCKETS
static void send_reply(CNCT connection, JRNH * msg, USHORT length)
{
/**************************************
 *
...
static void set_use_count(DJB database)
{
/**************************************
 *
...
static void sign_off(CNCT connection, LTJC * msg)
{
/**************************************
 *
 *	s i g n _ o f f
...
static void sign_on(CNCT connection, LTJC * msg)
{
/**************************************
 *
 *	s i g n _ o n
...
static DJB start_database(
						  ...
						  SCHAR * db_name,
						  SCHAR * dir_name,
						  SSHORT page_size, SLONG seqno, ULONG offset)
{
/**************************************
 *
...
static int start_server(UCHAR * journal_dir,
						USHORT sw_d, USHORT sw_v, USHORT sw_i)
{
/**************************************
 *
...
static BOOLEAN test_dup_entry(DJB dbb, SLONG file_id)
{
/**************************************
 *
...
static void time_stamp(GDS__QUAD * date)
{
/**************************************
 *
 *	t i m e _ s t a m p
