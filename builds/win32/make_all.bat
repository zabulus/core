...
if "%VS_VER%"=="msvc6" (
	...
	@devenv %ROOT_PATH%\builds\win32\%VS_VER%\Firebird2.sln %CLEAN% "release|%PLATFORM%" /OUT make_all.log
)
...
if "%VS_VER%"=="msvc6" (
	...
	@devenv %ROOT_PATH%\builds\win32\%VS_VER%\Firebird2.sln %CLEAN% "debug|%PLATFORM%" /OUT make_all.log
)
